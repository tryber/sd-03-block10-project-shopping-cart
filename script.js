  ...
  const price =
    Math.round(
      [...cartItem]
        ...
        .reduce((acc, priceItem) => acc + parseFloat(priceItem), 0) * 100
    ) / 100;
  ...
  await fetch(`https://api.mercadolibre.com/items/${sku}`)
    ...
    .then((add) =>
      document
        ...
        .appendChild(createCartItemElement(DontRepeat(add)))
    );
  await cardTotal();
  await update();
};
...
window.onload = async () => {
  ...
  document.getElementsByClassName(
    "cart__items"
  )[0].innerHTML = localStorage.getItem("Lista Salva");
  document.getElementsByClassName(
    "total-price"
  )[0].innerHTML = localStorage.getItem("Total a Pagar");
  document
    .querySelectorAll("li")
    .forEach((inner) =>
      inner.addEventListener("click", () => cartItemClickListener(inner))
    );
  document
